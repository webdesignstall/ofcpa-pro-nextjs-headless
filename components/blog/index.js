...
export default function Blog({blog}) {
    ...
    return (
        <div style={{overflow:"hidden"}}>
            ...
            <p>By <Link className='text-blue-500' href={`/author/${blog?.author?.slug}`}>{blog?.author?.name}</Link>  {moment(blog?.date).format('LL')} <Link href='#comment'>Write a Comment</Link> </p>
