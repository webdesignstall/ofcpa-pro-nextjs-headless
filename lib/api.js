...
const blogFields = `_id, title, 'slug': slug.current, coverImage, 'category': category->{_id, name, 'slug': slug.current}, 'author': author->{_id, name, 'slug': slug.current}, date`;
const seoFields = `{
      additionalMetaTags,
      metaDescription,
      metaTitle,
...
export async function getWorkingWithUs() {
    const workingWithUs = await client.fetch('*[_type == "workingWithUs"][0]{_id, sectionTitle, sectionSubtitle, items}')
    return workingWithUs
}


export async function getReviews() {
    const reviews = await client.fetch('*[_type == "review"]{_id, reviewerName, reviewerTitle, rating, reviewText}')
    return reviews
}

export async function getTaxCorporateServices() {
    const taxCorporateServices = await client.fetch('*[_type == "taxCorporateServices"][0]{_id,serviceTitle, serviceItems}')
    return taxCorporateServices
}

export async function getPackages() {
    const packages = await client.fetch('*[_type == "pricingPlan"]{_id, description, planName, price, features}')
    return packages
}
...
export async function getBlog(page = 1, limit = 10) {
...
export async function getAllBlog() {
    ...
    const allBlog = await client.fetch(
        `*[_type == "blog"] | order(date desc) {title, 'slug': slug.current}`
    );
    return allBlog;
}
