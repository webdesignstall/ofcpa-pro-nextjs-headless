...
// {title, slug, coverImage, category, author, date }
export async function getBlog() {
    const blog = await client.fetch(`*[_type == "blog"] | order(date desc){_id, title, 'slug': slug.current, coverImage, 'category': category->{name, 'slug': slug.current}, 'author': author->name, date, content }`)
    return blog
}